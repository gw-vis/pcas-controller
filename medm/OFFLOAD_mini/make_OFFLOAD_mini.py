...
header = '''
file {
	name="/opt/rtcds/userapps/release/cds/common/medm/steppingmotor/OFFLOAD_OVERVIEW/OFFLOAD_mini.adl"
	version=030107
}
display {
	object {
...
def head(x,y):
    ...
    "composite name"=""
    "composite file"="./HEAD_MINI.adl;IFO=$(IFO),ifo=$(ifo)"
    }}
    '''.format(common=common,x=x,y=y)
    return txt,width,height

def label(x,y,system):
    width = 40
    height = 30
    txt = '''
    composite {{
    ...
    "composite name"=""
    "composite file"="./LABEL_MINI.adl;IFO=$(IFO),ifo=$(ifo),SYSTEM={system}"
    }}
    '''.format(common=common,x=x,y=y,system=system)
    return txt,width,height

def info(x,y,system,stage,damp,dof):
    width = 10
    height = 30
    txt = '''
    composite {{
    ...
    "composite name"=""
    "composite file"="./INFO_MINI.adl;IFO=$(IFO),ifo=$(ifo),SYSTEM={system},STAGE={stage},DAMP={damp},DOF={dof}"
    }}
    '''.format(common=common,x=x,y=y,system=system,stage=stage,damp=damp,dof=dof)
    return txt,width,height
        ...
        for num,system in enumerate(systems):
            ...
            for stage in stages[system]:
                ...
                for dof in dofs[stage]:
                    ...
                    txt,w1,h1 = info(width+_w,height,system,sensor_stage,damp,sensor_dof)
                    _w += w1
                    contents += txt
            contents += txt
            height += _h0 + 10
